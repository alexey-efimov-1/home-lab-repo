...
#------------------------------------------------------------------------------
# Очистка старого host key из known_hosts (при пересоздании ВМ с тем же IP)
#------------------------------------------------------------------------------
KNOWN_HOSTS="${REAL_HOME}/.ssh/known_hosts"
if [[ -f "$KNOWN_HOSTS" ]]; then
    log "Удаление старого fingerprint для $VM_IP из $KNOWN_HOSTS..."
    ssh-keygen -f "$KNOWN_HOSTS" -R "$VM_IP" 2>/dev/null || true
fi
